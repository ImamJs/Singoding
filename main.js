...
btntampilkeranjang.addEventListener("click", () => {
  ...
  cart.forEach((item,index) => {
    ...
    <div class="bg-white flex justify-around rounded my-3 p-3"
      ...
      <button class="bg-red-500 px-3 py-1 rounded text-white" onclick="removeFromCart(${index})">Remove</button>
    </div>
  ...
  cart.forEach((item, i) => {
    keranjanganda.innerHTML += `
      <div class="bg-white flex justify-around rounded my-3 p-3">
        ...
        <button class="bg-red-500 px-3 py-1 rounded text-white" onclick="removeFromCart(${i})">Remove</button>
      </div>
    `;
  });

  // Perbarui jumlah keranjang di bilah navigasi
  const tampilKeranjangElement = document.querySelector(".tampil-keranjang i");
